...
async def get_account_by_login_password(
    pool, login: str, password: str
) -> Optional[Dict[str, Any]]:
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT id, login, password, is_blocked "
            "FROM account WHERE login = $1 AND password = $2",
            login,
            password,
        )
    return dict(row) if row else None
...
async def block_account(pool, account_id: int) -> None:
    async with pool.acquire() as conn:
        await conn.execute(
...
async def delete_account(pool, account_id: int) -> bool:
    async with pool.acquire() as conn:
        res = await conn.execute("DELETE FROM account WHERE id = $1", account_id)
    return res == "DELETE 1"
